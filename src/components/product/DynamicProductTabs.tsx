        ...
        {activeTab === "Specifications" && (
          <div className="max-w-3xl">
            ...
            <div className="rounded-lg border border-border overflow-hidden">
              {product.hsn_code && (
                <div className="flex items-start gap-4 px-5 py-3.5 text-sm bg-card">
                  <span className="font-medium text-foreground w-40 shrink-0">HSN Code</span>
                  <span className="text-foreground">{product.hsn_code}</span>
                </div>
              )}
              <div className="flex items-start gap-4 px-5 py-3.5 text-sm bg-secondary">
                <span className="font-medium text-foreground w-40 shrink-0">GST Rate</span>
                <span className="text-foreground">{product.gst_rate || 18}%</span>
              </div>
              <div className="flex items-start gap-4 px-5 py-3.5 text-sm bg-card">
                <span className="font-medium text-foreground w-40 shrink-0">Base Price</span>
          ...
          <div className="space-y-6 max-w-3xl">
            {customizationOptions.length === 0 ? (
              ...
              customizationOptions.map((opt) => {
                ...
                return (
                  <div key={opt.id} className="bg-card rounded-lg border border-border p-5">
                    <h4 className="font-semibold text-sm text-foreground mb-3">
                      {opt.option_label}
                      {opt.is_required && <span className="text-destructive ml-1">*</span>}
                    </h4>
                    <p className="text-xs text-muted-foreground mb-3">Type: {opt.option_type}</p>
              ...
              <>
                <div className="bg-card rounded-lg border border-border p-6 flex flex-col sm:flex-row items-center gap-6">
                  <div className="text-center shrink-0">
                    <p className="font-semibold text-5xl text-foreground">{reviewStats.averageRating}</p>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {reviews.map((r) => (
                    <div key={r.id} className="bg-card rounded-lg border border-border p-6">
                      ...
                      {r.review_body && <p className="text-sm text-muted-foreground leading-relaxed mb-4">"{r.review_body}"</p>}
                      ...
                      {r.admin_response && (
                        <div className="mt-3 p-3 rounded bg-secondary text-xs text-muted-foreground">
